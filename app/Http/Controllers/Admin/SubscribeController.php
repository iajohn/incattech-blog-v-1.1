...
class SubscribeController extends Controller{
    ...
    public function index(){
        return view('pages.frontend.subscribe.index')->with('profile', $this->authUser);
    }
    ...
    public function getAdminSubscriptionForm(){
        $user    = User::get(['email']);
        $title   = 'User\'s Subscription';
        $company =   Company::first()->get();
        return view('pages.backend.admin.subscription.new', compact('title','company'))->withUsers($user);
    }
    ...
    public function getSubscriptionHistory(Request $request){
        $user = new User();
        $user = $user->get(['email']);
        $title   = 'DevTv -  Subscription History';
        $company =   Company::first()->get();

        $history = $this->buildHistoryQuery($request)->get();
        return view('pages.backend.admin.subscription.index',compact('title','company'))->withHistorys($history)
            ->with('users', $user);
    }
    ...
    public function showSubscription($tranzid){
        ...
        $title   = 'DevTv -  Subscription Detail\'s';
        $company =   Company::first()->get();

        return view('pages.backend.admin.subscription.show',compact('title','company'))->with('detail', $subscription);
    }

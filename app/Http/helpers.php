...
function buildMenu($menu, $parentid = 0){
    ...
    foreach ($menu as $item)
        if ($item->parent_id == $parentid) {
            ...
            $result .= "<div class='dd-handle dd3-handle'>Drag</div><div class='dd3-content'>{$item->name}</div>";
            $result .= "<div class='actions'><a href='/admin/tv-channel/edit/{$item->id}' class='edit'>Edit</a>";
            $result .= "<a href='/admin/tv-channel/delete/{$item->id}' class='delete' ";
            $result .= "style='margin-left:10px'>Delete</a></div>";
            $result .= buildMenu($menu, $item->id) . "</li>";
        }
    return $result ?  "\n<ol class=\"dd-list\">\n$result</ol>\n" : null;
}
...
function getAllTags(){
    $home = new \App\Http\Controllers\PostController;
    return $home->getTags();
}

function getAllVideoTags(){
    $home = new \App\Http\Controllers\VidHomeController;
    return $home->getTags();
}

function getFrontEndCategories(){
    return buildFrontendTopMenu(\App\Channel::orderBy('id','asc')->get());
}
...
function buildFrontendTopMenu($menu, $parentid = 0){
    ...
    foreach ($menu as $item)
        if ($item->parent_id == $parentid) {
            $result .= "<li><a href=\"" .
                url('/tv/channel/' . $item->slug) .
                "\"><i class=\"fa fa-th\"></i>" . $item->name . "</a>";
            $result .= buildFrontendTopMenu($menu, $item->id) . "</li>";
        }

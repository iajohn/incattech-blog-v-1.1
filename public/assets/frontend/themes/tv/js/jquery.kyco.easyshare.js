...
kyco.easyShare = function() {
  ...
  if (jQuery('[data-easyshare]').length > 0) {
    jQuery('[data-easyshare]').each(function(index) {
      ...
      easyshares[index].countInstagram = easyshares[index].self.find('[data-easyshare-button-count="instagram"]');
      easyshares[index].countWhatsapp  = easyshares[index].self.find('[data-easyshare-button-count="whatsapp"]');
      ...
      // Get share counts for Facebook, Twitter, Instagram, Whatsapp, Google+, Linkedin, Pinterest and Xing
      jQuery.ajax({
        url: kyco.API_PATH,
        type: 'GET',
        dataType: 'json',
        data: {
          ...
          counts: {
            ...
            instagram: easyshares[index].countInstagram.length,
            whatsapp: easyshares[index].countWhatsapp.length,
        ...
        success: function(response) {
          ...
          easyshares[index].countInstagram.html(kyco.easyShareApproximate(response.Instagram));
          easyshares[index].countWhatsapp.html(kyco.easyShareApproximate(response.Whatsapp));
        ...
        error: function() {
          ...
          easyshares[index].countInstagram.html(0);
          easyshares[index].countWhatsapp.html(0);
      ...
      easyshares[index].self.find('[data-easyshare-button="instagram"]').click(function() {
        var width      = 500;
        var height     = 400;
        var leftOffset = (jQuery(window).width() - width) / 2;
        var topOffset  = (jQuery(window).height() - height) / 2;
        var url        = 'https://www.instagram.com/sharer/sharer.php?u=' + easyshares[index].SHARE_URL;
        var opts       = 'width=' + width + ',height=' + height + ',top=' + topOffset + ',left=' + leftOffset;
      ...
      easyshares[index].self.find('[data-easyshare-button="whatsapp"]').click(function() {
        // var text       = jQuery(this).data('easyshare-tweet-text') || '';
        var width      = 575;
        var height     = 440;
        var leftOffset = (jQuery(window).width() - width) / 2;
        var topOffset  = (jQuery(window).height() - height) / 2;
        var url        = 'https://api.whatsapp.com/share?text=' + encodeURIComponent(text);
        var opts       = 'status=1,width=' + width + ',height=' + height + ',top=' + topOffset + ',left=' + leftOffset;
